...
const messageData = [
  {
    ...
    message:
      ' Но синтетическое тестирование, в своём классическом представлении, допускает внедрение поэтапного и последовательного развития общества. В рамках спецификации современныхстандартов, сторонники тоталитаризма в науке будут функционально разнесены.',
  },
  {
    date: '31 января 2024',
    name: 'Тишкова Дарья',
  ...
  return (
    <div className="px-[25px] pt-[20px] pb-[25px] bg-white rounded-[10px] ">
      <div className="flex items-center gap-[25px] mb-[20px] ">
        <p className='className="text-[12px] md:text-[16px] lg:text-[18px] font-semibold'>
          Отзывы
      ...
      <div className="flex flex-col md:flex-row gap-[25px] justify-between ">
        <div className="flex flex-col gap-4 order-1 md:order-0">
          {messageData.slice(0, expend ? messageData.length : 2).map((message, index) => (
            ...
            >
              <div className="flex gap-4 items-center mb-1 ">
                <img src={messageImg} alt="image" className="w-[31px] h-[31px]  " />
                <p className="text-[12px] md:text-[14px] text-[--second] font-semibold ">
                  {message.name}
                </p>
                <span className="flex items-center justify-between  gap-1  ">
                  <span className="flex">
                    <IoIosStar className="text-yellow-500 text-[10px] md:text-[14px] "></IoIosStar>
                    <IoIosStar className="text-yellow-500 text-[10px] md:text-[14px] "></IoIosStar>
                    <IoIosStar className="text-yellow-500 text-[10px] md:text-[14px] "></IoIosStar>
                    <IoIosStar className="text-yellow-500 text-[10px] md:text-[14px] "></IoIosStar>
                    <IoIosStar className="text-yellow-500 text-[10px] md:text-[14px] "></IoIosStar>
          ...
          <div onClick={() => setExpend((prev) => !prev)} className="flex justify-center order-1">
            <BtnGG title={expend ? 'показывай меньше' : 'Показать еще'} />
          </div>
        </div>
