  ...
  return (
    <ModalCase modalKey="profileImg" >
      <div className="px-[25px] py-[20px]">
        ...
        <label htmlFor='profile' className='flex cursor-pointer mb-[50px] flex-col gap-[10px] justify-center items-center py-[77px] border border-dashed border-[--border] rounded-[10px] '>
          <label htmlFor='profile'>Выбор изображения профиля</label>
          <p className='text-[--pri] flex items-center gap-[10px] '> <HiCamera /> Выберите файл</p>
          <input type="file" name="profile" id="profile" hidden/>
        </label>

        <div className='flex justify-center'>
          <span onClick={()=>dispatch(hideModal('profileImg'))}> <BtnG title={"Отменить"}/> </span>
        </div>
      </div>
    </ModalCase>
  );
